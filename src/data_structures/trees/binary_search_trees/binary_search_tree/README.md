...
The BST links an ordering invariant to search performance: the invariant makes
every lookup a root-to-leaf path, and input order alone decides whether that
path is log n or n. Two-child deletion via the in-order successor is the
classic structural rewrite exercise, and doing it in safe Rust with owned
`Box` links — moving subtrees with `Option::take` rather than aliasing them —
is a genuine step up from the linked list.
