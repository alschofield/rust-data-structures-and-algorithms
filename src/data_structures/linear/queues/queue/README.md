...
The circular buffer is the classic fix for the naive array queue's O(n)
dequeue: modular index arithmetic makes both ends O(1). The subtle part is
growth — copying a wrapped buffer into a larger one without scrambling order —
which exercises careful index reasoning and, in Rust, correct handling of
initialized versus uninitialized slots.
