...
The stack is the entry point for ownership-aware container design in Rust:
`pop` returning `Option<T>` moves the element out instead of copying or
exposing a dangling reference, making the empty case impossible to ignore.
Building the growth logic by hand teaches amortized doubling and what `Vec`
does beneath its API.
